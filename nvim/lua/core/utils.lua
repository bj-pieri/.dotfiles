...
M.load_config = function()
   ...
   local customrc_exists = vim.fn.filereadable(vim.fn.stdpath "config" .. "/lua/custom/customrc.lua") == 1
   if customrc_exists then
      -- merge user config if it exists and is a table; otherwise display an error
      local user_config = require "custom.customrc"
      if type(user_config) == "table" then
         conf = vim.tbl_deep_extend("force", conf, user_config)
      else
         error "User config (customrc.lua) *must* return a table!"
      end
   end
...
-- remove plugins defined in customrc
