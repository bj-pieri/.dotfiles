...
utils.load_config = function()
   ...
   if chadrc_exists then
      ...
      if type(user_config) == "table" then
         conf.mappings = conf.mappings and utils.prune_key_map(conf.mappings, user_config.mappings, ignore_modes) or {}
         user_config.mappings = user_config.mappings and utils.prune_key_map(user_config.mappings, "rm_disabled", ignore_modes) or {}
...
utils.reduce_key_map = function(key_map, ignore_modes)
   local prune_keys = {}
...
utils.remove_disabled_mappings = function(key_map)
   local clean_map = {}
...
utils.prune_key_map = function(key_map, prune_map, ignore_modes)
   if not prune_map then return key_map end
   if not key_map then return prune_map end
   local prune_keys = type(prune_map) == "table" and utils.reduce_key_map(prune_map, ignore_modes)
       or { n = {}, v = {}, i = {}, t = {} }
            ...
            if prune_keys[mode] then
               ...
               for k, v in pairs(mappings) do
                  if not vim.tbl_contains(prune_keys[mode], k) then
                     filtered_mappings[k] = utils.remove_disabled_mappings(v)
                  end
               end
...
utils.no_WhichKey_map = function()
...
utils.remove_default_plugins = function(plugins)
   local removals = utils.load_config().plugins.remove or {}
...
utils.plugin_list = function(default_plugins)
   local user_plugins = utils.load_config().plugins.user
...
utils.load_override = function(default_table, plugin_name)
   local user_table = utils.load_config().plugins.override[plugin_name]
